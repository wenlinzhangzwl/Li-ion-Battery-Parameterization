clearvars -except meas meas_t pulse init ub lb
...
measResampled = meas_t(1:100:end, :);
...
xlabel('time[h]'); ylabel('SOC [%]'); title('SOC - dch'); grid minor
legend('sim', 'exp')
...
xlabel('Time [h]'); ylabel('Terminal Voltage [V]'); title('Voltage - dch'); grid minor
legend('exp', 'sim')

% Plot Voltage error
figure
plot(Batt.Time/3600, Batt.errV*1000);
xlabel('Time [h]'); ylabel('Error [mV]'); title('Voltage Error - dch'); grid minor
...
% hold off
% legend('R1', 'R2', 'R3', 'R0');
% title('Resistances - dch'); grid minor; xlabel('SOC [%]'); ylabel('R [mOhm]')
% 
% % Plot capacitances
% figure; 
% hold on
...
% hold off
% legend('C1', 'C2', 'C3');
% title('Capacitances - dch'); grid minor; xlabel('SOC [%]'); ylabel('C [kF]')

%% Save simulation results
% filename = append(folder, 'OCVRRC3_PUL25dch.mat');
...
function [Vt, SOC] = OCVRRCModel(Batt)
    ...
    param_t = table2array(Batt.pulse.Init); % ECM Parameters
    SOC_t = Batt.pulse.SOC;                 % SOC of ECM parameters
    ...
    for k = 2:length(Current)
        ...
        tau1 = interp1(SOC_t, param_t(:, 5), oSOC2, 'linear');
        tau2 = interp1(SOC_t, param_t(:, 6), oSOC2, 'linear');
        tau3 = interp1(SOC_t, param_t(:, 7), oSOC2, 'linear');
        oOCV = interp1(SOC_t, param_t(:, 8), oSOC2, 'linear');
